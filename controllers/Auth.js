...
exports.auth0Login = async (req, res) => {
    ...
    try {
        ...
        const auth0UserInfoResponse = await axios.get("https://dev-fzxjja1kyr5d58i2.us.auth0.com/userinfo", {
            headers: { Authorization: `Bearer ${token}` },
        });

        const auth0UserInfo = auth0UserInfoResponse.data;

        // Check if the user exists in your database
        let user = await User.findOne({ email: auth0UserInfo.email });
        ...
        if (!user) {
            ...
            user = await User.create({
                ...
                isVerified: true, // Auth0 users are already verified
            });
        }
        ...
        const appToken = generateToken(secureInfo);
        console.log(appToken, "apptoken...")

        res.cookie("token", appToken, {
            sameSite: process.env.PRODUCTION === "true" ? "None" : "Lax",
            maxAge: parseInt(process.env.COOKIE_EXPIRATION_DAYS, 10) * 24 * 60 * 60 * 1000, // Convert days to milliseconds
            httpOnly: true,
        ...
        if (error.response) {
            return res.status(error.response.status).json({ message: error.response.data.error || "Auth0 login failed" });
        }
...
exports.resendOtp = async (req, res) => {
    try {
        ...
        await sendMail(existingUser.email, `OTP Verification for Your E-commerce website account Account`, `Your One-Time Password (OTP) for account verification is: <b>${otp}</b>.</br>Do not share this OTP with anyone for security reasons`)
...
exports.forgotPassword = async (req, res) => {
    ...
    try {
        ...
        await sendMail(isExistingUser.email, 'Password Reset Link for Your E-commerce website account Account', `<p>Dear ${isExistingUser.name},

        We received a request to reset the password for your E-commerce website account account. If you initiated this request, please use the following link to reset your password:</p>
